...
# definition without updating flux_min to get all the elements
...
# definition with the update of flux_min to get all the elements
def CS_view2(nb=5, vars_names=["q","sersic"], show_axis=False, start=0, infos=True):
    """
    Params :
     - nb : size of the subplot
     - vars_names : variables to plot (y,x), between sersic, hlr, q, i, mag
    ...
    for i in range(nb):
        for j in range(nb):
            ...
            while not cond:
                ...
                if k==nb_data-1:
                    flux_min-=5 # reducing flux_min and restart the search
                    k=start
                k+=1
...
def show_image_from_file(idx, num_file, cmap='gray', infos=True):
    path = 'datas_full/COSMOS_25.2_training_sample/'
    ...
    if infos:
        ...
        print("size :",hdul_image[idx].shape)

    plt.imshow(hdul_image[idx].data, cmap=cmap)
    plt.axis("off")
    plt.show(block=False)
